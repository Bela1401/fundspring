...
export function ContributionPanel({ campaign }: { campaign: CampaignSummary }) {
  ...
  const amount = useMemo(() => {
    try {
      return amountInput && Number(amountInput) > 0 ? parseUnits(amountInput, 6) : 0n;
    } catch {
      return 0n;
    }
  }, [amountInput]);
  ...
  const { data: isEoa } = useQueryClientEoa(client, address);
  ...
  }, [address, amount, calls, campaign.address, chainId, client, memoValues, mode, needsApproval]);
  ...
  async function contribute() {
    ...
    if (tokenBalance < amount) {
      setTransaction({ phase: "error", message: "Your USDC balance is lower than the contribution amount." });
      return;
    }
    if (fee) {
      const nativeBalance = await client.getBalance({ address });
      const contributionAtNativePrecision = parseUnits(amountInput, 18);
      if (nativeBalance < contributionAtNativePrecision + fee) {
        setTransaction({
          phase: "error",
          message: "Add more USDC: the wallet must cover both the contribution and the estimated Arc fee.",
        });
        return;
      }
    }
    ...
    try {
      if (mode === "batch" && needsApproval) {
        ...
        setTransaction({ phase: "final", hash, message: "Atomic approval and contribution are final on Arc." });
      } else if (mode === "memo") {
        ...
        verifyContribution(receipt);
        setTransaction({ phase: "final", hash, message: "Contribution is final on Arc." });
      }
      await Promise.all([
        reads.refetch(),
  ...
  const busy = transaction.phase === "signing" || transaction.phase === "submitted";
  const eoaOnlyUnavailable = isEoa === false;
    ...
    <div className="panel p-5 md:p-6">
      ...
      <fieldset className="mt-5 grid gap-2">
        ...
        {([
          ...
          <label key={value} className={`flex items-center gap-3 rounded-xl border p-3 ${mode === value ? "border-lime-300/35 bg-lime-300/6" : "border-white/8"}`}>
            <input
              type="radio"
              name="route"
              checked={mode === value}
              disabled={value !== "standard" && eoaOnlyUnavailable}
      ...
      {mode === "memo" && (
        <label className="mt-4 block">
          ...
          <input className="field font-mono text-sm" value={reference} onChange={(event) => setReference(event.target.value.slice(0, 48))} />
      ...
      <button
        ...
        disabled={!isConnected || amount === 0n || busy || campaign.status !== 0}
        onClick={() => void contribute()}
      >
        {!isConnected ? "Connect wallet to contribute" : busy ? "Transaction in progress…" : "Contribute USDC"}
      </button>
